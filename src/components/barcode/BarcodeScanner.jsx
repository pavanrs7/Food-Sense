...
const BarcodeScanner = () => {
  ...
  // FIX 3: stopCamera fully cleans up the video element before returning
  const stopCamera = () => {
    ...
    if (videoRef.current) {
      ...
      videoRef.current.load(); // forces the element to fully release the source
    }
    setScanning(false);
    console.log("Camera stopped");
  };

  // FIX 1 + 2: startCamera stops existing stream, waits for hardware release,
  // and falls back to minimal constraints if needed
  const startCamera = async () => {
    setError(null);
    setCapturedImage(null);

    // Stop any existing stream and give hardware time to release
    stopCamera();
    ...
    try {
      ...
      try {
        // First attempt: prefer rear camera with ideal resolution
        ...
        // Fallback: minimal constraints — fixes NotReadableError on some devices
        console.warn(
          "Retrying with minimal constraints due to error:",
          constraintErr.name
        );
        stream = await navigator.mediaDevices.getUserMedia({
          video: true,
      ...
      if (err.name === "NotReadableError") {
        ...
        setError(
          `Camera error: ${err.message}. Please try again or enter the barcode manually.`
        );
      }
    }
  };
          ...
          searchPromises.push(
            fetch(
              `https://world.openfoodfacts.org/api/v2/search?categories_tags=${encodeURIComponent(
                categoryName
              )}&fields=code,product_name,nutrition_grades,image_url,brands,categories_tags&page_size=25`
            )
              .then((response) =>
  ...
  const getNutriScoreValue = (grade) => {
    ...
    return values[grade?.toLowerCase()] || 5;
  };
  ...
  // FIX 1: Check permission using Permissions API — no stream opened on mount
  useEffect(() => {
    navigator.permissions
      ?.query({ name: "camera" })
      .then((result) => {
        if (result.state === "granted") setCameraPermission(true);
        else if (result.state === "denied") setCameraPermission(false);
        // 'prompt' = unknown, leave as null
      ...
      .catch(() => {
        // Permissions API not supported in this browser — that's fine
      });
